...
void invertirSecuenciaBits(unsigned char *vectorBytes, int inicio, int fin) {
    ...
    __asm {
        ...
        MOV ECX, [EBP+12] // Mover variable 'inicio' a registro ECX
        MOV EDX, [EBP+16] // Mover variable 'fin' a registro EDX
        MOV EDI, [EBP+8] // Mover variable 'vectorBytes' a registro EDI
		startwhile:
        CMP ECX, EDX // while (inicio < fin)
        JGE finwhile

        // Push de las variables al llamado de la funcion
        PUSH EDX
        PUSH ECX
        PUSH EDI
        CALL intercambiarBits //intercambiarBits(vectorBytes, inicio, fin) intercambia los bits en las posiciones
        POP EDI
        POP ECX
        POP EDX
        INC ECX // inicio++
